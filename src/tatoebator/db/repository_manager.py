...
class SentenceRepository:
    def __init__(self, media_manager: MediaManager, external_download_requester: ExternalDownloadRequester):
        ...
        self._sentence_production_manager = SentenceProductionManager(external_download_requester)
        self._sentence_db_interface = SentenceDbInterface()
        self._ensure_starter_sentences()

    def _ensure_starter_sentences(self):
        if self._sentence_db_interface.count_n_sentences() < self._sentence_production_manager.amt_starter_sentences*0.5:
    ...
    def _produce_new_sentences_for_words(self, word_desired_amts: Dict[str, int], ensure_audio=False,
        ...
        def scoring_callback(e: ExampleSentence):
            words = e.lexical_words
            ...
            # -0.75 to request that ideally sentences have comprehensibility > 0.75
            return len(known_words) / len(words) - 0.75

        sentences = self._sentence_production_manager\
    ...
    def _ingest_starter_sentences(self, max_desired_sentences_per_word: int = SENTENCES_PER_WORD, block_size: int = 50):
        ...
        block = []
        sentence_text = set()
        for sentence in self._sentence_production_manager.yield_starter_sentences(filtering_fun=is_not_in_db):

            # check there is at least one lexical word which doesn't already exist a lot within the db
            least_present_lexical_word = min(self._sentence_db_interface.count_keywords(sentence.lexical_words).values())
